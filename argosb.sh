...
insuuid(){
if [ -z "$uuid" ] && [ ! -e "/app/agsb/uuid" ]; then
if [ -e "/app/agsb/sing-box" ]; then
uuid=$("/app/agsb/sing-box" generate uuid)
else
uuid=$("/app/agsb/xray" uuid)
fi
echo "$uuid" > "/app/agsb/uuid"
elif [ -n "$uuid" ]; then
echo "$uuid" > "/app/agsb/uuid"
fi
uuid=$(cat "/app/agsb/uuid")
echo "UUID密码：$uuid"
}
installxray(){
echo
echo "=========启用xray内核========="
if [ ! -e "/app/agsb/xray" ]; then
url="https://github.com/yonggekkk/ArgoSB/releases/download/argosbx/xray-$cpu"; out="/app/agsb/xray"; (command -v curl >/dev/null 2>&1 && curl -Lo "$out" -# --retry 2 "$url") || (command -v wget>/dev/null 2>&1 && wget -O "$out" --tries=2 "$url")
chmod +x "/app/agsb/xray"
sbcore=$("/app/agsb/xray" version 2>/dev/null | awk '/^Xray/{print $2}')
echo "已安装Xray正式版内核：$sbcore"
fi
cat > "/app/agsb/xr.json" <<EOF
{
  "log": {
    "access": "/dev/null",
    "error": "/dev/null",
...
echo "$ym_vl_re" > "/app/agsb/ym_vl_re"
echo "Reality域名：$ym_vl_re"
mkdir -p "/app/agsb/xrk"
if [ ! -e "/app/agsb/xrk/private_key" ]; then
key_pair=$("/app/agsb/xray" x25519)
private_key=$(echo "$key_pair" | head -1 | awk '{print $3}')
public_key=$(echo "$key_pair" | tail -n 1 | awk '{print $3}')
short_id=$(date +%s%N | sha256sum | cut -c 1-8)
echo "$private_key" > "/app/agsb/xrk/private_key"
echo "$public_key" > "/app/agsb/xrk/public_key"
echo "$short_id" > "/app/agsb/xrk/short_id"
fi
private_key_x=$(cat "/app/agsb/xrk/private_key")
public_key_x=$(cat "/app/agsb/xrk/public_key")
short_id_x=$(cat "/app/agsb/xrk/short_id")
fi
if [ -n "$xhp" ]; then
xhp=xhpt
if [ -z "$port_xh" ] && [ ! -e "/app/agsb/port_xh" ]; then
port_xh=$(shuf -i 10000-65535 -n 1)
echo "$port_xh" > "/app/agsb/port_xh"
elif [ -n "$port_xh" ]; then
echo "$port_xh" > "/app/agsb/port_xh"
fi
port_xh=$(cat "/app/agsb/port_xh")
...
if [ -n "$vlp" ]; then
vlp=vlpt
if [ -z "$port_vl_re" ] && [ ! -e "/app/agsb/port_vl_re" ]; then
port_vl_re=$(shuf -i 10000-65535 -n 1)
echo "$port_vl_re" > "/app/agsb/port_vl_re"
elif [ -n "$port_vl_re" ]; then
echo "$port_vl_re" > "/app/agsb/port_vl_re"
fi
port_vl_re=$(cat "/app/agsb/port_vl_re")
...
if [ -n "$ssp" ]; then
ssp=sspt
if [ ! -e "/app/agsb/sskey" ]; then
sskey=$(head -c 16 /dev/urandom | base64 -w0)
echo "$sskey" > "/app/agsb/sskey"
fi
if [ -z "$port_ss" ] && [ ! -e "/app/agsb/port_ss" ]; then
port_ss=$(shuf -i 10000-65535 -n 1)
echo "$port_ss" > "/app/agsb/port_ss"
elif [ -n "$port_ss" ]; then
echo "$port_ss" > "/app/agsb/port_ss"
fi
sskey=$(cat "/app/agsb/sskey")
port_ss=$(cat "/app/agsb/port_ss")
...
echo "=========启用Sing-box内核========="
if [ ! -e "/app/agsb/sing-box" ]; then
url="https://github.com/yonggekkk/ArgoSB/releases/download/argosbx/sing-box-$cpu"; out="/app/agsb/sing-box"; (command -v curl>/dev/null 2>&1 && curl -Lo "$out" -# --retry 2 "$url") || (command -v wget>/dev/null 2>&1 && wget -O "$out" --tries=2 "$url")
chmod +x "/app/agsb/sing-box"
sbcore=$("/app/agsb/sing-box" version 2>/dev/null | awk '/version/{print $NF}')
echo "已安装Sing-box正式版内核：$sbcore"
fi
cat > "/app/agsb/sb.json" <<EOF
{
"log": {
    "disabled": false,
    "level": "info",
...
EOF
insuuid
command -v openssl >/dev/null 2>&1 && openssl ecparam -genkey -name prime256v1 -out "/app/agsb/private.key" >/dev/null 2>&1
command -v openssl >/dev/null 2>&1 && openssl req -new -x509 -days 36500 -key "/app/agsb/private.key" -out "/app/agsb/cert.pem" -subj "/CN=www.bing.com" >/dev/null 2>&1
if [ ! -f "/app/agsb/private.key" ]; then
url="https://github.com/yonggekkk/ArgoSB/releases/download/argosbx/private.key"; out="/app/agsb/private.key"; (command -v curl>/dev/null 2>&1 && curl -Ls -o "$out" --retry 2 "$url") || (command -v wget>/dev/null 2>&1 && wget -q -O "$out" --tries=2 "$url")
url="https://github.com/yonggekkk/ArgoSB/releases/download/argosbx/cert.pem"; out="/app/agsb/cert.pem"; (command -v curl>/dev/null 2>&1 && curl -Ls -o "$out" --retry 2 "$url") || (command -v wget>/dev/null 2>&1 && wget -q -O "$out" --tries=2 "$url")
fi
if [ -n "$hyp" ]; then
hyp=hypt
if [ -z "$port_hy2" ] && [ ! -e "/app/agsb/port_hy2" ]; then
port_hy2=$(shuf -i 10000-65535 -n 1)
echo "$port_hy2" > "/app/agsb/port_hy2"
elif [ -n "$port_hy2" ]; then
echo "$port_hy2" > "/app/agsb/port_hy2"
fi
port_hy2=$(cat "/app/agsb/port_hy2")
...
cat >> "/app/agsb/sb.json" <<EOF
    {
        ...
        "tls": {
            ...
            "certificate_path": "/app/agsb/cert.pem",
            "key_path": "/app/agsb/private.key"
        }
    },
EOF
else
...
if [ -n "$tup" ]; then
tup=tupt
if [ -z "$port_tu" ] && [ ! -e "/app/agsb/port_tu" ]; then
port_tu=$(shuf -i 10000-65535 -n 1)
echo "$port_tu" > "/app/agsb/port_tu"
elif [ -n "$port_tu" ]; then
echo "$port_tu" > "/app/agsb/port_tu"
fi
port_tu=$(cat "/app/agsb/port_tu")
echo "Tuic端口：$port_tu"
cat >> "/app/agsb/sb.json" <<EOF
        {
            "type":"tuic",
...
if [ -n "$anp" ]; then
anp=anpt
if [ -z "$port_an" ] && [ ! -e "/app/agsb/port_an" ]; then
port_an=$(shuf -i 10000-65535 -n 1)
echo "$port_an" > "/app/agsb/port_an"
elif [ -n "$port_an" ]; then
echo "$port_an" > "/app/agsb/port_an"
fi
port_an=$(cat "/app/agsb/port_an")
...
echo "$ym_vl_re" > "/app/agsb/ym_vl_re"
echo "Reality域名：$ym_vl_re"
mkdir -p "/app/agsb/sbk"
if [ ! -e "/app/agsb/sbk/private_key" ]; then
key_pair=$("/app/agsb/sing-box" generate reality-keypair)
private_key=$(echo "$key_pair" | awk '/PrivateKey/ {print $2}' | tr -d '"')
public_key=$(echo "$key_pair" | awk '/PublicKey/ {print $2}' | tr -d '"')
short_id=$("/app/agsb/sing-box" generate rand --hex 4)
echo "$private_key" > "/app/agsb/sbk/private_key"
echo "$public_key" > "/app/agsb/sbk/public_key"
echo "$short_id" > "/app/agsb/sbk/short_id"
fi
private_key_s=$(cat "/app/agsb/sbk/private_key")
public_key_s=$(cat "/app/agsb/sbk/public_key")
short_id_s=$(cat "/app/agsb/sbk/short_id")
if [ -z "$port_ar" ] && [ ! -e "/app/agsb/port_ar" ]; then
port_ar=$(shuf -i 10000-65535 -n 1)
echo "$port_ar" > "/app/agsb/port_ar"
elif [ -n "$port_ar" ]; then
echo "$port_ar" > "/app/agsb/port_ar"
fi
port_ar=$(cat "/app/agsb/port_ar")
...
if [ -n "$vmp" ]; then
vmp=vmpt
if [ -z "$port_vm_ws" ] && [ ! -e "/app/agsb/port_vm_ws" ]; then
port_vm_ws=$(shuf -i 10000-65535 -n 1)
echo "$port_vm_ws" > "/app/agsb/port_vm_ws"
elif [ -n "$port_vm_ws" ]; then
echo "$port_vm_ws" > "/app/agsb/port_vm_ws"
fi
port_vm_ws=$(cat "/app/agsb/port_vm_ws")
echo "Vmess-ws端口：$port_vm_ws"
if [ -n "$cdnym" ]; then
echo "$cdnym" > "/app/agsb/cdnym"
...
xrsbout(){
if [ -e "/app/agsb/xr.json" ]; then
sed -i '${s/,\s*$//}' "/app/agsb/xr.json"
cat >> "/app/agsb/xr.json" <<EOF
  ],
  "outbounds": [
    {
...
EOF
nohup "/app/agsb/xray" run -c "/app/agsb/xr.json" >/dev/null 2>&1 &
fi
if [ -e "/app/agsb/sb.json" ]; then
sed -i '${s/,\s*$//}' "/app/agsb/sb.json"
cat >> "/app/agsb/sb.json" <<EOF
  ],
  "outbounds": [
    {
...
EOF
nohup "/app/agsb/sing-box" run -c "/app/agsb/sb.json" >/dev/null 2>&1 &
fi
}
killstart(){
...
nohup /app/agsb/sing-box run -c /app/agsb/sb.json >/dev/null 2>&1 &
nohup /app/agsb/xray run -c /app/agsb/xr.json >/dev/null 2>&1 &
if [ -e "/app/agsb/sbargotoken.log" ]; then
nohup /app/agsb/cloudflared tunnel --no-autoupdate --edge-ip-version auto --protocol http2 run --token $(cat /app/agsb/sbargotoken.log 2>/dev/null) >/dev/null 2>&1 &
else
if [ -e "/app/agsb/xr.json" ] && [ -e "/app/agsb/argo.log" ]; then
nohup /app/agsb/cloudflared tunnel --url http://localhost:$(grep -A2 vmess-xr /app/agsb/xr.json | tail -1 | tr -cd 0-9) --edge-ip-version auto --no-autoupdate --protocol http2 > /app/agsb/argo.log 2>&1 &
elif [ -e "/app/agsb/sb.json" ] && [ -e "/app/agsb/argo.log" ]; then
nohup /app/agsb/cloudflared tunnel --url http://localhost:$(grep -A2 vmess-sb /app/agsb/sb.json | tail -1 | tr -cd 0-9) --edge-ip-version auto --no-autoupdate --protocol http2 > /app/agsb/argo.log 2>&1 &
fi
fi
sleep 6
}
...
if [ ! -e "/app/agsb/cloudflared" ]; then
...
url="https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-$cpu"; out="/app/agsb/cloudflared"; (command -v curl>/dev/null 2>&1 && curl -Lo "$out" -# --retry 2 "$url") || (command -v wget>/dev/null 2>&1 && wget -O "$out" --tries=2 "$url")
chmod +x "/app/agsb/cloudflared"
fi
if [ -n "${ARGO_DOMAIN}" ] && [ -n "${ARGO_AUTH}" ]; then
name='固定'
nohup "/app/agsb/cloudflared" tunnel --no-autoupdate --edge-ip-version auto --protocol http2 run --token "${ARGO_AUTH}" >/dev/null 2>&1 &
echo "${ARGO_DOMAIN}" > "/app/agsb/sbargoym.log"
echo "${ARGO_AUTH}" > "/app/agsb/sbargotoken.log"
else
name='临时'
nohup "/app/agsb/cloudflared" tunnel --url http://localhost:"${port_vm_ws}" --edge-ip-version auto --no-autoupdate --protocol http2 > "/app/agsb/argo.log" 2>&1 &
fi
echo "申请Argo$name隧道中……请稍等"
sleep 8
if [ -n "${ARGO_DOMAIN}" ] && [ -n "${ARGO_AUTH}" ]; then
argodomain=$(cat "/app/agsb/sbargoym.log" 2>/dev/null)
else
argodomain=$(grep -a trycloudflare.com "/app/agsb/argo.log" 2>/dev/null | awk 'NR==2{print}' | awk -F// '{print $2}' | awk '{print $1}')
fi
...
echo '@reboot /bin/sh -c "nohup /app/agsb/sing-box run -c /app/agsb/sb.json >/dev/null 2>&1 &"' >> /tmp/crontab.tmp
fi
...
echo '@reboot /bin/sh -c "nohup /app/agsb/xray run -c /app/agsb/xr.json >/dev/null 2>&1 &"' >> /tmp/crontab.tmp
fi
sed -i '/agsb\/cloudflared/d' /tmp/crontab.tmp
if [ -n "$argo" ] && [ -n "$vmag" ]; then
if [ -n "${ARGO_DOMAIN}" ] && [ -n "${ARGO_AUTH}" ]; then
echo '@reboot /bin/sh -c "nohup /app/agsb/cloudflared tunnel --no-autoupdate --edge-ip-version auto --protocol http2 run --token $(cat /app/agsb/sbargotoken.log 2>/dev/null) >/dev/null 2>&1 &"' >> /tmp/crontab.tmp
else
if [ -e "/app/agsb/xray" ]; then
echo '@reboot /bin/sh -c "nohup /app/agsb/cloudflared tunnel --url http://localhost:$(grep -A2 vmess-xr /app/agsb/xr.json | tail -1 | tr -cd 0-9) --edge-ip-version auto --no-autoupdate --protocol http2 > /app/agsb/argo.log 2>&1 &"' >> /tmp/crontab.tmp
else
echo '@reboot /bin/sh -c "nohup /app/agsb/cloudflared tunnel --url http://localhost:$(grep -A2 vmess-sb /app/agsb/sb.json | tail -1 | tr -cd 0-9) --edge-ip-version auto --no-autoupdate --protocol http2 > /app/agsb/argo.log 2>&1 &"' >> /tmp/crontab.tmp
fi
fi
fi
...
server_ip="[$serip]"
echo "$server_ip" > "/app/agsb/server_ip.log"
else
server_ip="$serip"
echo "$server_ip" > "/app/agsb/server_ip.log"
fi
...
rm -rf "/app/agsb/jh.txt"
uuid=$(cat "/app/agsb/uuid")
server_ip=$(cat "/app/agsb/server_ip.log")
sxname=$(cat "/app/agsb/name" 2>/dev/null)
vmcdnym=$(cat "/app/agsb/cdnym" 2>/dev/null)
echo "*********************************************************"
echo "*********************************************************"
...
echo
if [ -e "/app/agsb/xray" ]; then
ym_vl_re=$(cat "/app/agsb/ym_vl_re" 2>/dev/null)
private_key_x=$(cat "/app/agsb/xrk/private_key" 2>/dev/null)
public_key_x=$(cat "/app/agsb/xrk/public_key" 2>/dev/null)
short_id_x=$(cat "/app/agsb/xrk/short_id" 2>/dev/null)
sskey=$(cat "/app/agsb/sskey" 2>/dev/null)
fi
if [ -e "/app/agsb/sing-box" ]; then
ym_vl_re=$(cat "/app/agsb/ym_vl_re" 2>/dev/null)
private_key_s=$(cat "/app/agsb/sbk/private_key" 2>/dev/null)
public_key_s=$(cat "/app/agsb/sbk/public_key" 2>/dev/null)
short_id_s=$(cat "/app/agsb/sbk/short_id" 2>/dev/null)
fi
if grep xhttp-reality "/app/agsb/xr.json" >/dev/null 2>&1; then
...
echo "$vl_xh_link" >> "/app/agsb/jh.txt"
echo "$vl_xh_link"
echo
fi
if grep reality-vision "/app/agsb/xr.json" >/dev/null 2>&1; then
...
echo "$vl_link" >> "/app/agsb/jh.txt"
echo "$vl_link"
echo
fi
if grep ss-2022 "/app/agsb/xr.json" >/dev/null 2>&1; then
...
echo "$ss_link" >> "/app/agsb/jh.txt"
echo "$ss_link"
echo
fi
if grep vmess-xr "/app/agsb/xr.json" >/dev/null 2>&1 || grep vmess-sb "/app/agsb/sb.json" >/dev/null 2>&1; then
...
echo "$vm_link" >> "/app/agsb/jh.txt"
echo "$vm_link"
echo
if [ -f "/app/agsb/cdnym" ]; then
...
echo "$vm_cdn_link" >> "/app/agsb/jh.txt"
echo "$vm_cdn_link"
echo
fi
fi
if grep anytls-sb "/app/agsb/sb.json" >/dev/null 2>&1; then
echo "💣【 AnyTLS 】节点信息如下："
port_an=$(cat "/app/agsb/port_an")
an_link="anytls://$uuid@$server_ip:$port_an?insecure=1&allowInsecure=1#${sxname}anytls-$hostname"
echo "$an_link" >> "/app/agsb/jh.txt"
echo "$an_link"
echo
fi
if grep anyreality-sb "/app/agsb/sb.json" >/dev/null 2>&1; then
...
echo "$ar_link" >> "/app/agsb/jh.txt"
echo "$ar_link"
echo
fi
if grep hy2-sb "/app/agsb/sb.json" >/dev/null 2>&1; then
echo "💣【 Hysteria2 】节点信息如下："
port_hy2=$(cat "/app/agsb/port_hy2")
hy2_link="hysteria2://$uuid@$server_ip:$port_hy2?security=tls&alpn=h3&insecure=1&sni=www.bing.com#${sxname}hy2-$hostname"
echo "$hy2_link" >> "/app/agsb/jh.txt"
echo "$hy2_link"
echo
fi
if grep tuic5-sb "/app/agsb/sb.json" >/dev/null 2>&1; then
echo "💣【 Tuic 】节点信息如下："
port_tu=$(cat "/app/agsb/port_tu")
...
echo "$tuic5_link" >> "/app/agsb/jh.txt"
echo "$tuic5_link"
echo
fi
argodomain=$(cat "/app/agsb/sbargoym.log" 2>/dev/null)
[ -z "$argodomain" ] && argodomain=$(grep -a trycloudflare.com "/app/agsb/argo.log" 2>/dev/null | awk 'NR==2{print}' | awk -F// '{print $2}' | awk '{print $1}')
if [ -n "$argodomain" ]; then
...
echo "$vmatls_link1" >> "/app/agsb/jh.txt"
...
echo "$vmatls_link2" >> "/app/agsb/jh.txt"
...
echo "$vmatls_link3" >> "/app/agsb/jh.txt"
...
echo "$vmatls_link4" >> "/app/agsb/jh.txt"
...
echo "$vmatls_link5" >> "/app/agsb/jh.txt"
...
echo "$vmatls_link6" >> "/app/agsb/jh.txt"
...
echo "$vma_link7" >> "/app/agsb/jh.txt"
...
echo "$vma_link8" >> "/app/agsb/jh.txt"
...
echo "$vma_link9" >> "/app/agsb/jh.txt"
...
echo "$vma_link10" >> "/app/agsb/jh.txt"
...
echo "$vma_link11" >> "/app/agsb/jh.txt"
...
echo "$vma_link12" >> "/app/agsb/jh.txt"
...
echo "$vma_link13" >> "/app/agsb/jh.txt"
sbtk=$(cat "/app/agsb/sbargotoken.log" 2>/dev/null)
if [ -n "$sbtk" ]; then
...
echo "$argoshow"
echo "---------------------------------------------------------"
echo "聚合节点信息，请查看/app/agsb/jh.txt文件或者运行cat /app/agsb/jh.txt进行复制"
echo "---------------------------------------------------------"
...
cleandel
rm -rf "/app/agsb"/{sb.json,xr.json,sbargoym.log,sbargotoken.log,argo.log,cdnym}
